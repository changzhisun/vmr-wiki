...
def frame_timestamps(video: Path, duration: float) -> list[float]:
    ...
    try:
        ...
        stamps = sorted(
            {
                round(float(packet["pts_time"]) - origin, 9)
    ...
    stamps = [
        stamp for stamp in stamps if math.isfinite(stamp) and 0 <= stamp < duration
    ]
    if not stamps:
        raise HarnessError(
            "Cannot determine source video frame presentation timestamps"
        )
    return stamps
...
def uniform_times(start: float, end: float, count: int, duration: float) -> list[float]:
    ...
    return sorted(
        {round(start + (end - start) * i / (count - 1), 9) for i in range(count)}
    )


def analyze_changes(
    video: Path, duration: float, config: dict, check_cancelled
) -> list[dict]:
                        ...
                        scores.append(
                            {
                                ...
                                "scene": sum(
                                    abs(a - b)
                                    for a, b in zip(histogram, previous_histogram)
                                )
                                / (2 * len(frame)),
                                "motion": sum(
                                    abs(a - b) for a, b in zip(frame, previous)
                                )
                                / (255 * len(frame)),
                            }
                        )
...
def mixed_times(
    ...
    *,
    ...
    count = min(
        config["max_frames"],
        max(2, math.ceil((end - start) / config["min_sample_interval_sec"])),
    )
    if global_scan:
    ...
    for kind, budget in (
        ("scene", scene_budget),
        ("motion", count - len(selected) - scene_budget),
    ):
        added = 0
        for row in sorted(candidates, key=lambda r: (-r[kind], r["timestamp"])):
            if row[kind] <= 0 or (
                kind == "scene" and row[kind] < config["scene_threshold"]
            ):
                continue
            near = [
                row["timestamp"],
            ...
            for stamp in near if kind == "scene" else near[:1]:
                ...
                stamp = round(stamp, 9)
                distance = (
                    min(separation, offset / 2) if kind == "scene" else separation
                )
...
def context_bounds(
    segments: list[dict], index: int, start: float, end: float, overlap: float
) -> tuple[float, float]:
    ...
    left = (
        overlap
        * min(length, segments[index - 1]["end"] - segments[index - 1]["start"])
        / 2
        if index
        else 0
    )
