...
def test_any_gt_hit_missing_denominator_and_abstention(tmp_path):
    gt = [{"query_id": qid, "video_id": "v", "moments": [moment(0, 1), moment(2, 3)]}
          for qid in ["a", "b", "c"]]
    pred = [{"query_id": "a", "video_id": "v", "moments": [moment(2, 3)]},
            {"query_id": "b", "video_id": "v", "moments": []}]
...
def test_qvh_length_groups_and_top_ten_cap():
    gt = {"q": {"query_id": "q", "video_id": "v", "moments": [moment(0, 10), moment(10, 40), moment(40, 100)]}}
...
def test_aggregate_validates_and_preserves_multiple_moments(tmp_path):
    ...
    inputs.mkdir()
    row = {"query_id": "q", "video_id": "v", "moments": [moment(0, 1), moment(2, 3)]}
    write_json(inputs / "q.json", row)
    assert aggregate(inputs, tmp_path / "all.jsonl") == [row]
    with pytest.raises(HarnessError):
        aggregate(inputs, tmp_path / "all.jsonl", max_predictions=1)


def test_unknown_and_duplicate_predictions_rejected(tmp_path):
    truth = {"query_id": "q", "video_id": "v", "moments": [moment(0, 1)]}
