...
from vmr.core.validation import cli, identifier, unique_index
...
def aggregate(
    ...
    *,
    ...
    dataset, split, videos, queries, saved, experiment = experiment_context(
        input_dir.parent, dataset_dir, split
    )
    ...
    for path in sorted(input_dir.iterdir()):
        ...
        prediction = validate_result(
            read_json(path), query_index, videos, split, max_predictions
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Aggregate one dataset/split; reject mixed predictions"
    )
    parser.add_argument("--input", required=True, type=Path)
    parser.add_argument("--output", required=True, type=Path)
    parser.add_argument(
        "--dataset", help="Dataset name; defaults to saved experiment identity"
    )
    parser.add_argument(
        "--split",
        help="Exact dataset.json split name; defaults to saved experiment split",
    )
    parser.add_argument(
        "--config",
        type=Path,
        help="Config for dataset paths; otherwise use experiment config",
    )
