...
def temporal_iou(a: dict, b: dict) -> float:
    intersection = max(
        0.0, min(a["end_sec"], b["end_sec"]) - max(a["start_sec"], b["start_sec"])
    )
...
def retrieval_metrics(
    predictions: dict, ground_truth: dict, top_k: list[int], thresholds: list[float]
) -> dict:
...
def average_precision(
    ...
    for rank, prediction in enumerate(predictions, 1):
        overlaps = [
            (temporal_iou(prediction, gt), i) for i, gt in enumerate(ground_truth)
        ]
