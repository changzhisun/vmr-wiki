...
from vmr.core.validation import nonempty
...
class Aliases:
    ...
    def __init__(self, secret: str, split: str, queries: list[dict]):
        ...
        self.query = {
            q["query_id"]: alias(secret, "query", q["query_id"]) for q in queries
        }
        self.video = {
            q["video_id"]: alias(secret, "video", q["video_id"]) for q in queries
        }
    ...
    def task(self, query: dict, max_predictions: int, duration: float) -> dict:
        ...
        return {
            "query_id": self.query[query["query_id"]],
