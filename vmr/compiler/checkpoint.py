...
class CompileCheckpoint:
    def __init__(
        self, output: Path, identity: dict, *, namespace=".compile-checkpoints"
    ):
        # Outside videos/: incomplete checkpoints cannot look like wiki outputs.
        self.root = (
            output.parent.parent / namespace / output.name / object_hash(identity)
        )
    ...
    def seal_image(self, frame: dict, elapsed_sec: float):
        self.write(
            ...
            {
                "frame": frame,
                "sha256": file_hash(self.root / frame["frame"]),
    ...
    def record_attempt(self, window_id: str, attempt: dict):
        ...
        count = (
            max((int(path.stem) for path in directory.glob("*.json")), default=-1) + 1
        )
