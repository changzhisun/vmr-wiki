...
"""
        process = subprocess.run(
            [sys.executable, "-c", script], cwd=workspace, capture_output=True
        )
        ...
        return AgentResult(
            7 if self.behavior == "nonzero" else process.returncode,
...
def test_structured_trace_keeps_readable_final_answers(tmp_path):
    ...
    assert (
        readable_trace("claude_code", b"legacy plain output\n")
...
def test_readable_stdout_is_written_even_when_container_cleanup_fails(
...
def test_trace_and_stdout_exist_when_agent_setup_fails(cfg, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        runner,
        "_docker",
        lambda *args, **kwargs: (_ for _ in ()).throw(HarnessError("network failed")),
    )
...
def test_all_prediction_ends_over_duration_are_clamped():
    ...
    status = run_status(
        {"query_id": "q1", "status": "success", "output_adjustments": adjustments},
        Path("results/test"),
    )
...
def test_experiment_requires_frozen_split(prepared):
    ...
    with pytest.raises(HarnessError, match="ingested but not frozen") as exc:
        Experiment(cfg, "test", runner=ProcessFixtureRunner())
    assert "python harness/freeze.py --dataset qvhighlights --split train" in str(
        exc.value
    )
...
@pytest.mark.parametrize(
    "behavior",
    ["missing", "invalid", "extra", "nonzero", "timeout", "symlink", "mutate"],
)
...
def test_workspace_hides_public_identifiers_and_saves_real_ones(frozen):
    ...
    task = seen[0]
    assert (
        task["query_id"] != query["query_id"] and task["video_id"] != query["video_id"]
    )
...
def test_aliases_are_stable_within_and_distinct_across_experiments(frozen):
    ...
    class Spy(ProcessFixtureRunner):
        def run(self, workspace, prompt, stdout, stderr):
            tasks.setdefault(self.behavior, []).append(
                read_json(workspace / "task.json")["query_id"]
            )
            return super().run(workspace, prompt, stdout, stderr)
    ...
    with Experiment(cfg, "one", runner=Spy("one")) as experiment:
        assert (
            experiment.aliases.query[experiment.queries[0]["query_id"]]
            == tasks["one"][0]
        )
...
def test_wiki_naming_its_video_is_refused(frozen, tmp_path):
    ...
    cfg, _ = frozen
    current = (
        Path(cfg["paths"]["wiki"]) / "qvhighlights" / "videos" / "video" / "wiki.md"
    )
...
def test_internal_prediction_processing_error_stays_a_harness_failure(
    frozen, monkeypatch
):
    cfg, _ = frozen
    monkeypatch.setattr(
        "harness.run_query.clamp_prediction_ends",
        lambda *args, **kwargs: (_ for _ in ()).throw(
            HarnessError("internal clamp invariant")
        ),
    )
    with Experiment(cfg, "internal-error", runner=ProcessFixtureRunner()) as experiment:
        ...
        metadata = read_json(
            experiment.root / "run_metadata" / f"{query['query_id']}.json"
        )
...
def test_discarded_tampered_prediction_does_not_claim_adjustments(frozen):
    ...
    with Experiment(
        cfg, "tampered-adjustment", runner=OverrunAndTamperRunner()
    ) as experiment:
        result = experiment.run(experiment.queries[0])
...
def test_interrupted_attempt_is_retried_not_charged_to_the_agent(frozen):
    ...
    with Experiment(cfg, "test", runner=runner) as experiment:
        query = experiment.queries[0]
        write_json(
            experiment.root / "run_metadata" / "1.json",
            {
                "query_id": "1",
                "video_id": "video",
...
def test_docker_mount_boundary_and_credentials_not_in_command(
    ...
    assert mounts == [
        f"type=bind,src={tmp_path},dst=/workspace,readonly",
    ...
    cfg["query"].update(
        load_config(
            Path(__file__).resolve().parents[1] / "config.yaml",
            query_agent_profile="claude_code",
        )["query"]
    )
    cfg["query"]["model"] = "fixture-agent"
    monkeypatch.setenv("AGENT_API_KEY", "another-secret")
...
def test_claude_tool_surface_allows_writing_and_excludes_network_tools(
    cfg, monkeypatch
):
    monkeypatch.setenv("AGENT_API_KEY", "test-secret")
    monkeypatch.setattr(
    ...
    denied = command[
        command.index("--disallowedTools") + 1 : command.index(
...
def written_config(tmp_path, **profile) -> Path:
    ...
    raw["profiles"]["agent"].update(profile)
...
def test_text_only_defaults_off_and_rejects_non_boolean(tmp_path):
    ...
    path.write_text(yaml.safe_dump(raw))
    with pytest.raises(
        HarnessError, match="query.input_mode must be text or multimodal"
    ):
        load_config(path)
...
def test_base_url_defaults_to_the_vendor_endpoint(tmp_path):
    ...
    del raw["profiles"]["agent"]["base_url"]
    path = tmp_path / "config.yaml"
    path.write_text(yaml.safe_dump(raw))
    assert load_config(path)["query"]["base_url"] == {"claude_code": None}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "not in profiles.agent.egress_allowed_hosts",
        ),
...
def test_base_url_matching_the_allowlist_is_accepted(tmp_path):
    ...
    assert (
        load_config(path)["query"]["base_url"]["claude_code"]
