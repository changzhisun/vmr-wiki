...
from vmr.core.validation import (
    identifier,
    nonempty,
    number,
    positive_int,
...
TEMPLATES = {"agents": "wiki_agents.md", "prompt": "wiki_prompt.md"}
...
DEFAULTS = {
    ...
    "api_key_env": {"codex": "AGENT_API_KEY", "claude_code": "AGENT_API_KEY"},
...
def settings(ingest: dict, templates: Path | None = None) -> dict:
    ...
    if model == "AGENT_MODEL" or model.startswith("REPLACE_"):
        raise HarnessError(
            "Set AGENT_MODEL or configure an explicit ingest.agentic.model"
        )
    ...
    if not isinstance(hosts, dict):
        raise HarnessError(
            "ingest.agentic.egress_allowed_hosts must be a mapping of agent to hosts"
        )
    if not isinstance(result["api_key_env"], dict) or not isinstance(
        result["base_url"], dict
    ):
    ...
    if extraction["strategy"] not in STRATEGIES:
        raise HarnessError(
            "ingest.agentic.frame_extraction.strategy must be adaptive or uniform"
        )
    ...
    if extraction["min_interval_sec"] > extraction["initial_interval_sec"]:
        raise HarnessError(
            "frame_extraction.min_interval_sec must not exceed initial_interval_sec"
        )
    ...
    if not isinstance(levels, list) or not levels or len(set(levels)) != len(levels):
        raise HarnessError(
            "ingest.agentic.wiki.levels must be a list of distinct level names"
        )
    if any(level not in LEVELS for level in levels):
        raise HarnessError(
            f"ingest.agentic.wiki.levels must be chosen from {', '.join(LEVELS)}"
        )
    ...
    if levels != [level for level in LEVELS if level in levels]:
        raise HarnessError(
            "ingest.agentic.wiki.levels must stay in chapter, event, moment order"
        )
    ...
    if not isinstance(names, dict) or set(names) != {"agents", "prompt"}:
        raise HarnessError("ingest.agentic.templates must set agents and prompt")
    result["templates"] = {
        "agents": relative_template(names["agents"], "ingest.agentic.templates.agents"),
        "prompt": relative_template(names["prompt"], "ingest.agentic.templates.prompt"),
    }
    recorded = raw.get("templates_hash")
    if templates is not None:
        result["templates_hash"] = template_hash(templates, result["templates"])
    elif recorded is None:
        raise HarnessError(
            "Stored agentic settings have no templates_hash; use a new wiki root"
        )
    else:
