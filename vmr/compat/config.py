...
from vmr.config.resolve import _mapping


from vmr.config.resolve import _known


from vmr.config.loader import _merge


from vmr.config.loader import _load_extended


from vmr.config.resolve import (
    agent_kind,
    agent_settings,
    _agent_profiles,
    select_named_agent,
...
from vmr.config.resolve import _vlm_profiles


from vmr.config.resolve import _select_profile
...
def _normalize_v2(
    ...
    *,
    ...
    _known(
        storage,
        {"root", "datasets", "wikis", "runs", "results", "templates"},
        "storage",
    )
    ...
    named = "agents" in profiles
    _known(profiles, {"agents" if named else "agent", "vlms"}, "profiles")
    if named:
        agents = _agent_profiles(profiles["agents"])
    else:
        agent = agent_settings(profiles.get("agent"))
    ...
    _known(
        media, {"sample_interval_sec", "image_max_size", "jpeg_quality"}, "wiki.media"
    )
    ...
    try:
        ...
        raise HarnessError(
            f"Missing configuration field: wiki.media.{exc.args[0]}"
        ) from exc

    if method == "agentic":
        if "captioner_profile" in wiki:
            raise HarnessError(
                "wiki.captioner_profile is not used by the agentic method"
            )
        if named:
            agent = _select_profile(
                agents, wiki.get("agent_profile"), "wiki.agent_profile"
            )
            kind = agent["kind"]
        else:
            kind = agent_kind(wiki.get("kind"), "wiki.kind")
        injected = {
            "agent",
            "model",
            "container_image",
        ...
        if set(method_config) & injected:
            raise HarnessError(
                "wiki.method_config must not repeat settings owned by profiles.agent"
            )
        ingest["agentic"] = {
            **deepcopy(method_config),
            ...
            "api_key_env": {kind: agent["api_key_env"]},
            "base_url": {kind: agent["base_url"]},
            "egress_allowed_hosts": {kind: agent["egress_allowed_hosts"]},
        }
    else:
        selector = "agent_profile" if named else "kind"
        if selector in wiki:
            raise HarnessError(f"wiki.{selector} is only valid for the agentic method")
    ...
    if named:
        ...
        if query_agent and query_agent_profile and query_agent != query_agent_profile:
            raise HarnessError("--agent and --agent-profile select different kinds")
        kind = agent_kind(
            query_agent or query_agent_profile or query.get("kind"), "query.kind"
        )
    ...
    names = query_templates(query.get("templates"), text_only=False)
    text_names = query_templates(query.get("templates"), text_only=True)
    return {
...
from vmr.config.resolve import hostname


from vmr.config.resolve import endpoint_host
...
def load_config(
    ...
    *,
    ...
    if version == 2:
        cfg = _normalize_v2(
            raw, path, query_agent=query_agent, query_agent_profile=query_agent_profile
        )
    else:
        ...
        if query_agent is not None:
            try:
                ...
                raise HarnessError(
                    f"Missing or malformed configuration field: {exc}"
                ) from exc
        if warn_legacy:
            warnings.warn(
    ...
    try:
        ...
        for name in ("datasets", "wiki", "runs", "results", "templates"):
            cfg["paths"][name] = str(
                (path.parent / nonempty(cfg["paths"][name], name)).resolve()
            )
        ingest = cfg["ingest"]
        ingest.setdefault("consecutive_failure_limit", 3)
        positive_int(
            ingest["consecutive_failure_limit"], "ingest.consecutive_failure_limit"
        )
        ...
        if ingest["caption_mode"] == "bidirectional":
            ...
            if cache_dir is not None:
                ingest["bidirectional"]["shared_cache_dir"] = str(
                    (path.parent / cache_dir).resolve()
                )
        if ingest["caption_mode"] == "hierarchical":
            from harness.hierarchy_config import HIERARCHY_VERSION, settings

            if (
                ingest.get("hierarchy_processing_version", HIERARCHY_VERSION)
                != HIERARCHY_VERSION
            ):
                raise HarnessError(
                    "Unsupported hierarchy_processing_version; use a new wiki root"
                )
        ...
        if ingest["caption_mode"] == "agentic":
            ...
            normalized = [
                hostname(host, f"ingest.agentic.egress_allowed_hosts.{agent}")
                for host in hosts
            ]
            if len(set(normalized)) != len(normalized):
                raise HarnessError(
                    f"ingest.agentic.egress_allowed_hosts.{agent} contains duplicates"
                )
            ...
            if agentic["base_url"][agent] is not None:
                host = endpoint_host(
                    agentic["base_url"][agent], f"ingest.agentic.base_url.{agent}"
                )
        ...
        if ingest["dense_timestamp_mode"] not in ("absolute_seconds", "frame_index"):
            raise HarnessError(
                "dense_timestamp_mode must be absolute_seconds or frame_index"
            )
        # Code-owned version: cannot silently run new rules under an old identity.
        if ingest.get("caption_processing_version", 4) != 4:
            raise HarnessError(
                "caption_processing_version is unsupported; use a new wiki root"
            )
        ...
        if (
            ingest["caption_mode"] == "dense"
            and ingest["caption_window_frames"] > 1
        ...
        if (
            type(ingest["caption_max_repairs"]) is not int
            or ingest["caption_max_repairs"] < 0
        ):
        ...
        if ingest["caption_mode"] != "agentic":
            ...
            if (
                ingest["caption_mode"] in ("hierarchical", "bidirectional")
                and timeline_fields
            ):
                raise HarnessError(
        ...
        if not isinstance(query.get("api_key_env"), dict):
            raise HarnessError(
                "query.api_key_env must be a mapping of agent to environment variable"
            )
        if not isinstance(query.get("egress_allowed_hosts"), dict):
            raise HarnessError(
                "query.egress_allowed_hosts must be a mapping of agent to hosts"
            )
        ...
        if not isinstance(hosts, list) or not hosts:
            raise HarnessError(
                f"query.egress_allowed_hosts.{agent} must be a nonempty list"
            )
        normalized = [
            hostname(host, f"query.egress_allowed_hosts.{agent}") for host in hosts
        ]
        if len(set(normalized)) != len(normalized):
            raise HarnessError(
                f"query.egress_allowed_hosts.{agent} contains duplicates"
            )
...
def main() -> None:
    ...
    explain = subparsers.add_parser(
        "explain", help="Print the normalized runtime configuration"
    )
