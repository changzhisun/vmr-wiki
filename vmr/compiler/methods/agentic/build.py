...
from vmr.compiler.methods.agentic.validate import validate_agent_wiki
from vmr.core.errors import HarnessError
from vmr.core.hashing import file_hash
from vmr.core.time import now
from vmr.core.jsonio import parse_json, write_json
...
def build_task(
    cfg: dict, duration: float, video_stream_duration: float, media: dict
) -> dict:
...
def publish_agentic(
    ...
    *,
    ...
    names = options["templates"]
    instructions = (templates / names["agents"]).read_text(encoding="utf-8")
    prompt = (templates / names["prompt"]).read_text(encoding="utf-8")
    ...
    logs = Path(
        cfg["paths"].get("logs", output.parent.parent / ".ingest-logs" / video_id)
    )
    ...
    try:
        ...
        if (
            ...
            and _missing_required(job / "output")
            and remaining >= MIN_REPAIR_SEC
        ):
            check()
            LOG.warning(
                "%s: agent exited without wiki.md/frames.jsonl; retrying once "
                "(%.0fs remaining); logs: %s",
                video_id,
                remaining,
                logs,
            )
            result = _run_agent(
                ...
                job,
                REPAIR_PROMPT,
                logs / "agent.repair.stdout.log",
                logs / "agent.repair.stderr.log",
                video=video,
                scratch=scratch,
                cancel_event=cancel_event,
        ...
        rows = validate_agent_wiki(
            ...
            max_frames=options["max_frames"],
            max_wiki_bytes=options["max_wiki_bytes"],
            max_frame_bytes=options["max_frame_bytes"],
            forbidden_tokens=forbidden,
            required_levels=tuple(options["wiki"]["levels"]),
        )

        validated = {name: file_hash(job / "output" / name) for name in PUBLISHED}
        validated.update(
            {row["frame"]: file_hash(job / "output" / row["frame"]) for row in rows}
        )
        ...
        try:
            ...
            LOG.warning(
                "Published Wiki; could not remove checkpoint: %s", checkpoint.root
            )
        return metadata
    finally:
        remove_tree(job)
