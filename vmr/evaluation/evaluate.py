...
from vmr.core.errors import FAILURE_KINDS, HARNESS_FAILURE_KINDS, HarnessError
from vmr.core.validation import cli, identifier, number, positive_int, unique_index
from vmr.core.hashing import file_hash, object_hash
from vmr.core.jsonio import read_json, read_jsonl, write_json
from vmr.datasets.manifest import (
    load_dataset,
    load_query_inputs,
...
from vmr.evaluation.metrics import retrieval_metrics
from vmr.evaluation.results import (
    check_identity,
    experiment_context,
    validate_result,
    verify_bundle,
...
def evaluate(
    ...
    *,
    ...
    allow_harness_failures: bool = False,
) -> dict:
    ...
    verify_bundle(
        pred_path, dataset, split, queries, required=not allow_unverified_predictions
    )
    ...
    if metadata_dir is not None:
        ...
        for path in sorted(metadata_dir.glob("*.json")):
            ...
            if status != "success":
                ...
                kind = (
                    "interrupted" if status == "running" else meta.get("failure_kind")
                )
    ...
    run_failures = (
        failure_breakdown(kinds, truth, statuses) if metadata_dir is not None else None
    )
    ...
    result = {
        ...
        "average_predictions_per_query": sum(
            len(p["moments"]) for p in predictions.values()
        )
        / len(truth),
        "top_k": list(top_k),
        "iou_thresholds": list(iou_thresholds),
        "retrieval": retrieval_metrics(
            predictions, truth, list(top_k), list(iou_thresholds)
        ),
        "inputs": {
            "prediction_sha256": file_hash(pred_path),
            "ground_truth_sha256": file_hash(gt_path),
            "dataset_hash": object_hash(dataset),
            "queries_hash": object_hash(queries),
        },
    }
    if run_failures is not None:
        result["run_failures"] = run_failures
    result.update(
        get_evaluator(evaluator)(predictions, truth, official_root=official_root)
    )
    ...
    if scored and scored.keys() != truth.keys():
        subset = {
            ...
            "average_predictions_per_query": sum(
                len(predictions[qid]["moments"]) for qid in scored
            )
            / len(scored),
            "retrieval": retrieval_metrics(
                predictions, scored, list(top_k), list(iou_thresholds)
            ),
        }
        subset.update(
            get_evaluator(evaluator)(predictions, scored, official_root=official_root)
        )
...
def main():
    from vmr.compat.config import dataset_path, load_config

    parser = argparse.ArgumentParser(
        description="Evaluate one declared split; unlabeled splits can only generate predictions"
    )
    parser.add_argument(
        "--dataset", help="Dataset name; defaults to saved experiment or config"
    )
    parser.add_argument(
        "--split",
        help="Exact split name; saved config or default_eval_split when omitted",
    )
    parser.add_argument("--pred", required=True, type=Path)
    parser.add_argument(
        "--gt", type=Path, help="Optional GT path inside the selected dataset directory"
    )
    parser.add_argument("--output", type=Path)
    parser.add_argument("--config", type=Path)
    parser.add_argument(
        "--evaluator", help="Evaluator adapter override; defaults to dataset.json"
    )
    parser.add_argument(
        "--official-root",
        type=Path,
        help="Official evaluator checkout understood by the dataset adapter",
    )
    ...
    directory = (
        args.gt.parent
        if args.gt is not None and args.dataset is None
        else dataset_path(cfg, "datasets")
    )
    if args.dataset:
    ...
    print(
        f"{output}: {result['dataset']}/{result['split']}, {result['num_queries']} queries, {result['failed_runs']} failed runs"
    )
    if "successful_only" in result:
        print(
            f"  scored over {result['successful_only']['num_queries']} answered queries as well; "
