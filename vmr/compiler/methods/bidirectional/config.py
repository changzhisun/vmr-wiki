...
from vmr.core.validation import number, positive_int
...
def settings(ingest: dict) -> dict:
    ...
    for group, keys in (
        ("topdown", ("enabled",)),
        ("bottomup", ("enabled", "independent_observation")),
    ...
    if (
        not result["topdown"]["enabled"]
        and not result["bottomup"]["independent_observation"]
    ):
        raise HarnessError("Contextual bottom-up requires topdown")
    if (
        not 1
        <= positive_int(result["max_frames_per_call"], "max_frames_per_call")
        <= 100
    ):
        raise HarnessError("max_frames_per_call must be in [1,100]")
    for group, keys in (
        ("topdown", ("max_depth", "max_children_per_node")),
    ...
    for group, key in (
        ("topdown", "min_segment_duration_sec"),
        ("bottomup", "window_sec"),
        ("reconciliation", "window_sec"),
        ("boundary_refinement", "uncertainty_sec"),
    ...
    if (
        number(
            result["boundary_refinement"]["context_expand_ratio"],
    ...
    if result["shared_cache_dir"] is not None and not isinstance(
        result["shared_cache_dir"], str
    ):
...
def budgets(options, duration):
    ...
    raw = options["budget"]
    return {
        "max_requests": raw["max_requests"] or hours * raw["requests_per_hour"],
