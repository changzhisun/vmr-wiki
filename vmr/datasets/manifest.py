...
from vmr.core.validation import identifier, nonempty, number, unique_index
from vmr.core.jsonio import read_json, read_jsonl
from vmr.datasets.validation import validate_moment, validate_query


MIGRATION = (
    "Re-run the Dataset Adapter to create dataset.json and split-bearing manifests."
)
...
def load_dataset(directory: Path, expected_name: str | None = None) -> dict:
    ...
    if expected_name is not None and metadata["name"] != expected_name:
        raise HarnessError(
            f"Dataset name mismatch: expected {expected_name!r}, got {metadata['name']!r}"
        )
    return metadata
...
def validate_videos(rows: list[dict], metadata: dict, split: str) -> dict[str, dict]:
    ...
    for video in rows:
        ...
        if set(video) != {"video_id", "video_path", "duration", "split"}:
            raise HarnessError(
                "Video must contain video_id, video_path, duration, split"
            )
...
def load_query_inputs(
    directory: Path, split: str, metadata: dict | None = None
    ...
    queries = [
        row
        for row in read_jsonl(directory / "queries.jsonl")
        if row_split(row, metadata) == split
    ]
    ...
    for query in queries:
        ...
        if query["video_id"] not in videos:
            raise HarnessError(
                f"Query video {query['video_id']!r} is not in split {split!r}"
            )
...
def require_ground_truth(metadata: dict, split: str) -> None:
    if not metadata["splits"][split]["has_ground_truth"]:
        raise HarnessError(
            f"Split {split!r} has_ground_truth=false: local evaluation is unavailable; "
            "this split can only generate predictions."
        )


def validate_ground_truth(
    rows: list[dict], metadata: dict, split: str, videos: dict, queries: list[dict]
) -> dict:
    truth = unique_index(
        [row for row in rows if row_split(row, metadata) == split], "query_id"
    )
    query_index = unique_index(queries, "query_id")
    if truth.keys() != query_index.keys():
        raise HarnessError(
            f"GT and query IDs must match exactly within split {split!r}"
        )
