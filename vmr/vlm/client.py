...
from vmr.core.validation import nonempty, positive_int, number
from vmr.vlm.transport import (
    FatalVLMError,
    ResponseRejected,
    check_cancelled,
    endpoint_gate,
...
class VLMClient:
    def __init__(
        ...
        *,
        ...
        self.gate = endpoint_gate(
            config["base_url"],
            positive_int(
                config.get("max_concurrent_requests", 4), "max_concurrent_requests"
            ),
        )
        self.max_retry_delay = number(
            config.get("max_retry_delay_sec", 60), "max_retry_delay_sec", 0.001
        )
        self.queue_timeout = number(
            config.get("queue_timeout_sec", 300), "queue_timeout_sec", 0.001
        )
        ...
        if config["model"] == "VLM_MODEL" or config["model"].startswith("REPLACE_"):
            raise FatalVLMError("Set VLM_MODEL or configure an explicit VLM model")
        if config["base_url"] == "VLM_BASE_URL":
            raise FatalVLMError(
                "Set VLM_BASE_URL or configure an explicit VLM base URL"
            )
    ...
    def caption(
        ...
        *,
        ...
        if text_only and (paths or prompt_override is None):
            raise HarnessError(
                "Text-only calls require an explicit prompt and no images"
            )
        ...
        prompt = (
            ...
            else nonempty(prompt_override, "prompt_override")
        )
        ...
        if timestamps is not None:
            ...
            if prompt.count(placeholder) != 1:
                raise HarnessError(
                    "VLM timestamp prompt must contain exactly one placeholder"
                )
            ...
            prompt = prompt.replace(
                placeholder, ", ".join(f"{timestamp}" for timestamp in timeline)
            )
            if target_timestamps is not None:
                ...
                if not targets or any(target not in timeline for target in targets):
                    raise HarnessError(
                        "VLM target timeline must be a nonempty subset of timestamps"
                    )
                prompt += (
                    "\n\nCentered target region: "
        ...
        for attempt in range(cfg["max_retries"] + 1):
            ...
            try:
                with self.gate.slot(self.cancel_event, self.queue_timeout) as waited:
                    ...
                    try:
                        ...
                        if exc.code in (429, 503):
                            ...
                            trace["shared_cooldown_sec"] = self.gate.defer(
                ...
                if finish_reason in _REFUSAL_REASONS:
                    raise HarnessError(
                        f"VLM caption was refused (finish_reason={finish_reason!r})"
                    )
                ...
                if exc.code in (401, 403, 404):
                    raise FatalVLMError(
                        f"VLM request failed with HTTP {exc.code}; check shared credentials, endpoint and model"
                    ) from None
                if delay > self.max_retry_delay:
                    raise HarnessError(
                        f"Server Retry-After exceeds max_retry_delay_sec: requested {delay:g}s, "
                        f"limit {self.max_retry_delay:g}s; resume later"
                    ) from None
                if (
                    ...
                    raise HarnessError(
                        f"VLM request failed with HTTP {exc.code}"
                    ) from None
            except (
                ...
                if attempt == cfg["max_retries"]:
                    raise HarnessError(
                        "VLM request failed: connection error or timeout"
                    ) from None
            except (
                KeyError,
                IndexError,
                ...
                if attempt == cfg["max_retries"]:
                    raise HarnessError(
                        "VLM returned an invalid caption response after bounded retries"
                    ) from exc
            except HarnessError:
