...
from vmr.core.jsonio import read_jsonl
...
def _validate_files(
    output: Path, max_wiki_bytes: int, max_frame_bytes: int, max_frames: int
) -> dict[str, Path]:
    ...
    for path in _entries(frames_dir):
        if not path.is_file():
            raise HarnessError(
                f"output/frames must contain only image files: {path.name}"
            )
        if path.stat().st_size == 0:
            raise HarnessError(f"Empty frame image: {path.name}")
        if path.stat().st_size > max_frame_bytes:
            raise HarnessError(
                f"Frame image exceeds {max_frame_bytes} bytes: {path.name}"
            )
    ...
    if len(images) > max_frames:
        raise HarnessError(
            f"Agent produced {len(images)} frames; the limit is {max_frames}"
        )
    return images


def _validate_registry(
    output: Path, images: dict[str, Path], duration: float
) -> list[dict]:
    ...
    for index, row in enumerate(rows, 1):
        ...
        if missing:
            raise HarnessError(
                f"frames.jsonl line {index} is missing {', '.join(missing)}"
            )
        ...
        if not isinstance(frame_id, str) or not FRAME_ID.fullmatch(frame_id):
            raise HarnessError(
                f"frames.jsonl line {index}: frame_id must match ^f[0-9]{{6}}$"
            )
        ...
        if frame not in images:
            raise HarnessError(
                f"frames.jsonl line {index} registers a missing image: {frame}"
            )
        ...
        for field in ("entities", "objects"):
            if field in row and (
                ...
                raise HarnessError(
                    f"frames.jsonl line {index}: {field} must be a list of strings"
                )
...
def _validate_wiki(
    ...
    for prefix in FORBIDDEN_PREFIXES:
        if prefix in text:
            raise HarnessError(
                f"wiki.md contains a non-portable absolute path: {prefix}"
            )
...
def validate_agent_wiki(
    ...
    *,
    ...
    leak = _leaked(
        (output / "frames.jsonl").read_text(encoding="utf-8"), forbidden_tokens
    )
    if leak is not None:
        raise HarnessError(
