...
VARIANTS = (
    ("simple_1_1", "simple", 1, 1),
    ("simple_5_1", "simple", 5, 1),
    ("dense_5_1", "dense", 5, 1),
    ("dense_9_4", "dense", 9, 4),
)
SIMPLE_PROMPT = (
    ...
    return object_hash(
        {
            ...
            for folder in ("harness", "adapters", "agents", "vmr")
...
def prepare(
    cfg: dict, output: Path, *, split: str, limit_videos: int = 10, seed: int = 0
):
    ...
    selected = sorted(
        {q["video_id"] for q in queries}, key=lambda vid: object_hash([seed, vid])
    )[:limit_videos]
    selected = set(selected)
    subset_queries = [q for q in queries if q["video_id"] in selected]
    if not subset_queries:
        raise HarnessError(
            "Selected videos have no queries; choose another seed or larger subset"
        )
    subset_rows = []
    ...
    if "{{FRAME_TIMESTAMPS}}" not in dense_prompt:
        method = yaml.safe_load(
            (
                ...
                / "configs/compiler/methods/dense.yaml"
            ).read_text(encoding="utf-8")
        )
        dense_prompt = method["profiles"]["vlms"]["qwen_default"]["prompt"].strip()
    configs = {}
    for name, mode, window, stride in VARIANTS:
        variant = deepcopy(cfg)
        variant["dataset"] = {"name": metadata["name"], "split": split}
        variant["paths"].update(
            datasets=str(output / "datasets"),
            wiki=str(output / "wiki" / name),
            results=str(output / "results"),
            runs=str(output / "runs" / name),
        )
        variant["ingest"].update(
            caption_mode=mode,
            caption_window_frames=window,
            caption_stride_frames=stride,
            dense_timestamp_mode="absolute_seconds",
        )
        variant["ingest"]["vlm"]["prompt"] = (
            dense_prompt if mode == "dense" else SIMPLE_PROMPT
        )
    ...
    template_names = query_templates(
        cfg.get("query_templates") or cfg["query"].get("templates"),
        text_only=cfg["query"].get("text_only", False),
    )
    manifest = {
        "version": 1,
        ...
        "video_hashes": {
            row["video_id"]: file_hash(Path(row["video_path"])) for row in subset_rows
        },
        "seed": seed,
        "video_ids": sorted(selected),
        "num_queries": len(subset_queries),
        "configs": configs,
        "dataset_files": {p.name: file_hash(p) for p in snapshot.iterdir()},
        "templates": {
            p: file_hash(Path(cfg["paths"]["templates"]) / p)
            for p in (template_names["agents"], template_names["prompt"])
        },
    }
...
def load_suite(root: Path):
    ...
    for row in read_jsonl(directory / "videos.jsonl"):
        if (
            file_hash(Path(row["video_path"]))
            != manifest["video_hashes"][row["video_id"]]
        ):
...
def run(root: Path, stage: str, jobs: int):
    ...
    from vmr.query.batch import run_queries, validate_jobs
    from vmr.query.experiment import Experiment

    if stage == "compile":
        stage = "ingest"  # legacy measurement key retained for saved reports
    ...
    for name, cfg in configs.items():
        app = from_legacy(cfg)
        set_path = root / "wiki_sets" / f"{name}.json"
        ...
        for step in ("ingest", "query", "evaluate"):
            ...
            try:
                if step == "ingest":
                    ...
                    with Experiment(
                        app.query,
                        dataset=dataset_path(cfg, "datasets"),
                        split=manifest["split"],
                        wiki_set=set_path,
                        root=result_root,
                        templates=app.storage.templates,
                        runs=app.storage.runs,
                    ) as experiment:
                        runtime_path = root / "runtime.json"
                        if (
                            runtime_path.exists()
                            and read_json(runtime_path) != experiment.runner.provenance
                        ):
                            raise HarnessError(
                                "Ablation agent runtime changed; prepare a new suite"
                            )
                        write_json(runtime_path, experiment.runner.provenance)
                        bar = ProgressBar(
                            len(experiment.queries),
                            desc=f"Querying {name}",
                            unit="queries",
                            log_stream="stdout",
                        )
                        bar.start()
                    ...
                    metrics = evaluate(
                        prediction,
                        dataset_dir=dataset_path(cfg, "datasets"),
                        split=manifest["split"],
...
def summarize(root: Path) -> dict:
    ...
    for name, cfg in configs.items():
        ...
        set_path = root / "wiki_sets" / f"{name}.json"
        if set_path.exists():
            wiki_set = WikiSet.open(set_path)
            for vid in manifest["video_ids"]:
                artifact = wiki_set.artifact(vid)
                telemetry.append(
                    artifact.manifest.data["provenance"]["build"].get("telemetry", {})
                )
        result_root = Path(cfg["paths"]["results"]) / name
        ...
        if metrics is not None:
            if (metrics["dataset"], metrics["split"], metrics["num_queries"]) != (
                manifest["dataset"],
                manifest["split"],
                manifest["num_queries"],
            ):
                raise HarnessError(f"Ablation metrics do not match suite: {name}")
            if metrics["inputs"]["prediction_sha256"] != file_hash(
                result_root / "predictions.jsonl"
            ):
                raise HarnessError(
                    f"Ablation predictions changed since evaluation: {name}"
                )
            if (
                metrics["inputs"]["ground_truth_sha256"]
                != manifest["dataset_files"]["ground_truth.jsonl"]
            ):
                raise HarnessError(
                    f"Ablation ground truth does not match metrics: {name}"
                )
        statuses = [
            read_json(p).get("status")
            for p in (result_root / "run_metadata").glob("*.json")
        ]
        finished_queries = sum(s in ("success", "failed") for s in statuses)
        tokens_complete = bool(telemetry) and all(
            t.get("api_requests", 0) > 0
            and t.get("api_requests") == t.get("requests_with_total_tokens")
            for t in telemetry
        )
        measurement = root / "measurements" / f"{name}.json"
        rows.append(
            {
                "variant": name,
                "ingested_videos": len(telemetry),
                "finished_queries": finished_queries,
                "complete": len(telemetry) == len(manifest["video_ids"])
                and finished_queries == manifest["num_queries"]
                and metrics is not None,
                "metrics": metrics,
                "stage_wall_sec": read_json(measurement)
                if measurement.exists()
                else None,
                "caption_attempts": sum(t.get("caption_attempts", 0) for t in telemetry)
                if telemetry
                else None,
                "rejected_attempts": sum(
                    t.get("rejected_attempts", 0) for t in telemetry
                )
                if telemetry
                else None,
                "extraction_sec": sum(t.get("extraction_sec", 0) for t in telemetry)
                if telemetry
                else None,
                "caption_sec": sum(t.get("caption_sec", 0) for t in telemetry)
                if telemetry
                else None,
                "tokens_complete": tokens_complete,
                "caption_total_tokens": sum(
                    t.get("usage", {}).get("total_tokens", 0) for t in telemetry
                )
                if tokens_complete
                else None,
            }
    ...
    return {
        ...
        "split": manifest["split"],
        "num_videos": len(manifest["video_ids"]),
        "num_queries": manifest["num_queries"],
        "note": "Costs cover completed videos, including their retained failed attempts. "
        "Unavailable token usage is null; no model pricing is assumed.",
        "variants": rows,
    }
...
def main():
    ...
    prep = sub.add_parser(
        "prepare", help="Create matched configs and dataset snapshot; no API calls"
    )
    ...
    execute = sub.add_parser(
        "run", help="Run real caption/query APIs for a prepared suite"
    )
    execute.add_argument("--suite", type=Path, required=True)
    execute.add_argument(
        "--stage", choices=("all", "ingest", "query", "evaluate"), default="all"
    )
    ...
    if args.action == "prepare":
        ...
        print(
            f"Prepared {len(result['configs'])} variants, {result['num_queries']} queries at {args.output}"
        )
