...
from vmr.core.jsonio import parse_json, read_json, write_json
from vmr.compiler.methods.bidirectional.config import PIPELINE_VERSION
from vmr.vlm.transport import ResponseRejected
...
def batches(records, max_records, max_chars):
    ...
    for record in records:
        ...
        if cost > max_chars:
            raise HarnessError(
                "One semantic record exceeds the configured text batch budget"
            )
...
class FrameIndex:
    def __init__(self, db, video, duration, checkpoint, staging, ingest, check):
        ...
        self.checkpoint, self.staging, self.ingest, self.check = (
            checkpoint,
            staging,
            ingest,
            check,
            ...
            try:
                while True:
                    ...
                    for line in lines:
                        ...
                        if 0 <= stamp < self.duration:
                            self.db.execute(
                                "INSERT OR IGNORE INTO source_frames VALUES(?)",
                                (stamp,),
                            )
                ...
                if (
                    ...
                    or not self.db.execute(
                        "SELECT 1 FROM source_frames LIMIT 1"
                    ).fetchone()
                ):
    ...
    def times(self, start, end, cap):
        count = self.db.execute(
            "SELECT COUNT(*) FROM source_frames WHERE timestamp>=? AND timestamp<?",
            (start, end),
        ).fetchone()[0]
        if count <= cap and count:
            return [
                ...
                for row in self.db.execute(
                    "SELECT timestamp FROM source_frames WHERE timestamp>=? AND timestamp<? ORDER BY timestamp",
                    (start, end),
                )
            ]
        selected = set()
        for i in range(cap if count else 1):
            ...
            before = self.db.execute(
                """SELECT timestamp FROM source_frames
    ...
    def images(self, start, end, cap):
        ...
        for stamp in self.times(start, end, cap):
            ...
            row = self.db.execute(
                "SELECT data FROM frames WHERE id=?", (frame_id,)
            ).fetchone()
            frame = {
                "frame_id": frame_id,
                "timestamp": stamp,
            ...
            if row is None:
                ...
                self.db.execute(
                    ...
                    (frame_id, stamp, canonical(frame).decode(), elapsed),
                )
            frames.append(frame)
        self.db.commit()
        return frames
...
class RequestJournal:
    ...
    def ensure_budget(self):
        if self.count >= self.limits["max_requests"]:
            raise HarnessError(
                "Bidirectional max_requests exhausted; checkpoint retained, no partial Wiki published"
            )
    ...
    def request(self, role, payload, instruction, parser, *, frames=(), replicate=None):
        ...
        def validate(value):
            try:
                ...
                raise HarnessError(
                    f"Malformed structured response ({type(exc).__name__}): {exc}"
                ) from exc
        ...
        identity = {
            ...
            "vlm": {
                key: self.cfg["ingest"]["vlm"][key]
                for key in ("model", "temperature", "max_tokens")
            },
            "images": [
                file_hash(self.checkpoint.root / frame["frame"]) for frame in frames
            ],
        }
        ...
        if saved is not None:
            ...
            def invoke(text, images, kind):
                ...
                try:
                    ...
                    if isinstance(exc, HarnessError) and not isinstance(
                        exc, ResponseRejected
                    ):
                ...
                try:
                    ...
                    while True:
                        try:
                            ...
                            format_repairs += 1
                            raw = invoke(
                                "Repair only JSON syntax in the following original answer. Preserve all "
                    ...
                    if attempt == repairs:
                        raise HarnessError(
                            f"{role}: response repair budget exhausted; checkpoint retained: {exc}"
                        ) from exc
                    correction = (
                        "\nYour structured answer was rejected: "
                        + str(exc)
                ...
                canonical(
                    {
                        ...
                        "shared_cache": saved.get("shared_cache")
                        or (key if shared_hit else None),
        ...
        if frames:
            self.db.execute(
                "INSERT INTO sampling(data) VALUES(?)",
                (
                    canonical(
                        {
                            "request_id": role + "/" + key,
    ...
    def telemetry(self):
        ...
        totals = {
            ...
            for key in (
                ...
                "caption_sec",
                "api_requests",
                "requests_with_total_tokens",
                "rejected_attempts",
            )
        }
        totals["usage"] = {
            key: sum(stage["usage"][key] for stage in stages.values())
