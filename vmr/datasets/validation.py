...
def validate_prediction(
    ...
    *,
    ...
    if (
        ...
        or prediction.keys() - required - {"evidence"}
    ):
        raise HarnessError(
            "Prediction requires query_id, video_id, split, moments; evidence is optional"
        )
    nonempty(prediction["split"], "split")
    if split is not None and prediction["split"] != split:
        raise HarnessError(
            f"Mismatched split: expected {split!r}, got {prediction['split']!r}"
        )
    ...
    if not isinstance(moments, list) or len(moments) > max_predictions:
        raise HarnessError(
            f"moments must be an array of at most {max_predictions} entries"
        )
    previous_score = 1.0
    for moment in moments:
        validate_moment(moment, duration)
        required_moment = {"start_sec", "end_sec", "score"}
        if not required_moment <= moment.keys() or moment.keys() - required_moment - {
            "evidence"
        }:
            raise HarnessError(
                "Moment requires start_sec, end_sec, score; evidence is optional"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate an agent prediction without repairing it"
    )
    parser.add_argument("prediction", type=Path)
    parser.add_argument("--query-id")
    parser.add_argument("--video-id")
    parser.add_argument(
        "--split", help="Expected exact split string (no alias mapping)"
    )
