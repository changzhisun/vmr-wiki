...
def agentic_config(tmp_path: Path, **agentic) -> Path:
    ...
    raw["storage"].update(
        {kind: str(tmp_path / kind) for kind in ("datasets", "runs", "results")}
    )
...
def agentic(tmp_path):
    ...
    QVHighlightsAdapter().prepare(
        raw, video_root, Path(cfg["paths"]["datasets"]) / "qvhighlights", split="train"
    )
    yield cfg
    ...
    return [
        {
            ...
            "frame": f"frames/{i:06d}.jpg",
            "reason": "semantic_evidence",
            "description": f"A red scene, sample {i}.",
        }
...
def wiki_text(rows):
    evidence = "\n".join(
        f"- [{row['frame_id']} @ {row['timestamp']}s]({row['frame']})" for row in rows
    )
    return (
...
def test_agentic_wiki_flows_through_freeze_and_query(agentic):
    ...
    assert sorted(p.name for p in wiki.iterdir()) == [
        "frames",
        "frames.jsonl",
        "ingest.json",
        "wiki.md",
    ...
    with Experiment(cfg, "agentic", runner=PredictingRunner()) as experiment:
        assert all(
            experiment.run(query)["status"] == "success" for query in experiment.queries
        )
...
def test_contract_violations_are_never_published(agentic, behavior, message):
    ...
    report = read_json(
        Path(cfg["paths"]["wiki"]) / "qvhighlights" / ".ingest-failures" / "video.json"
    )
...
def test_agentic_config_needs_no_vlm_deployment(tmp_path):
    ...
    raw["profiles"]["agent"]["model"] = "fixture-agent-model"
...
def test_agentic_config_rejects_unpinned_model_and_unreachable_gateway(
    tmp_path, monkeypatch
):
    monkeypatch.delenv("AGENT_MODEL", raising=False)
    with pytest.raises(HarnessError, match="AGENT_MODEL"):
        load_config(agentic_config(tmp_path, model="AGENT_MODEL"))
    with pytest.raises(
        HarnessError, match="not in profiles.agent.egress_allowed_hosts"
    ):
        load_config(
            agentic_config(
                tmp_path,
                agent="codex",
                base_url={
                    "codex": "https://elsewhere.example.org/v1",
                    "claude_code": None,
                },
            )
        )
    with pytest.raises(
        HarnessError, match="levels must stay in chapter, event, moment order"
    ):
...
def test_agentic_instructions_use_dense_temporal_observation_and_prioritize_complete_output():
    ...
    assert (
        "必须保持原始宽高比" in instructions
        and "force_original_aspect_ratio" in instructions
    )
    assert "宽和高都必须小于 2000 像素" in instructions
    assert instructions.index(
        "### 3. 像 Dense Caption 一样记录原子变化"
    ) < instructions.index("### 4. 保留密集覆盖并生成完整产物")
...
def test_container_budget_is_provenance_not_content(agentic):
    ...
    relaxed["ingest"]["agentic"].update(
        timeout_sec=7200, memory_gb=16, cpus=8, container_image="other:local"
    )
    assert ingest_content_hash(relaxed) == ingest_content_hash(cfg)
    assert ingest_content_diff(relaxed["ingest"], cfg) == []


def test_ingest_mounts_the_video_readonly_and_keeps_credentials_out(
    agentic, monkeypatch, tmp_path
):
    cfg = agentic
    monkeypatch.setenv("AGENT_API_KEY", "test-secret")
    monkeypatch.setattr(
        AgentIngestRunner, "_inspect", staticmethod(lambda _: "sha256:fixture")
    )
    runner = AgentIngestRunner(cfg)
    job, video, scratch = tmp_path / "job", tmp_path / "v.mp4", tmp_path / "scratch"
    command = runner.docker_command(
        job, "test-container", "isolated-network", video=video, scratch=scratch
    )
    ...
    assert mounts == [
        f"type=bind,src={job},dst=/workspace,readonly",
...
def test_missing_credential_is_a_shared_failure_not_one_video(agentic, monkeypatch):
    ...
    with pytest.raises(FatalAgentError, match="AGENT_API_KEY"):
        ingest_all(cfg, jobs=2)
    assert not (
        Path(cfg["paths"]["wiki"]) / "qvhighlights" / ".ingest-failures"
    ).exists()
