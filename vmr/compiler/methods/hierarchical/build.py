...
from vmr.media.sampling import (
    analyze_changes,
    context_bounds,
    frame_timestamps,
    mixed_times,
...
SCHEMA = {
    ...
    "summary": "Factual visual description",
    "actors": ["person"],
    "actions": ["adds"],
    "objects": ["tomato", "pan"],
    "state_before": "Tomato on board",
    "state_after": "Tomato in pan",
    "confidence": 0.9,
}
...
def semantic_fields(value: dict) -> dict:
    ...
    for key in ("actors", "actions", "objects"):
        ...
        result[key] = list(
            dict.fromkeys(nonempty(item, key).strip() for item in value[key])
        )
...
def parse_split(
    text: str, start: float, end: float, max_children: int, *, global_scan=False
):
    ...
    if payload["terminal"]:
        if global_scan or nodes:
            raise HarnessError(
                "Only a recursive semantic leaf may return terminal=true, nodes=[]"
            )
        return []
    ...
    for node in nodes:
        if not isinstance(node, dict) or set(node) != SEGMENT_KEYS:
            raise HarnessError(
                "Each segment must match the supplied node schema exactly"
            )
        left, right = number(node["start"], "start"), number(node["end"], "end")
        if abs(left - boundary) > 1e-6 or not left < right or right > end + 1e-6:
            raise HarnessError(
                "Nodes must form a chronological, gap-free, nonoverlapping target partition"
            )
...
def parse_merge(text: str, siblings: list[dict]):
    ...
    for group in payload["groups"]:
        ...
        if (
            ...
            or members != ids[cursor : cursor + len(members)]
        ):
            raise HarnessError(
                "Merge groups must partition adjacent sibling IDs exactly once, in order"
            )
...
def validate_tree(rows: list[dict], duration: float):
    ...
    for row in rows:
        ...
        if parent_id is None:
            ...
            if (
                parent is None
                or LEVELS.index(row["level"]) != LEVELS.index(parent["level"]) + 1
            ):
    ...
    for parent_id, siblings in children.items():
        start, end = (
            (0.0, duration)
            if parent_id is None
            else (by_id[parent_id]["start"], by_id[parent_id]["end"])
        )
        cursor = start
...
def render_tree(rows: list[dict], duration: float, frame_count: int) -> str:
    ...
    lines = [
        ...
        "- Caption mode: hierarchical",
        f"- Semantic nodes: {len(rows)}",
    ...
    for row in rows:
        if row["parent_id"] is None:
            lines.append(
                f"- `{row['node_id']}` {row['start']:g}–{row['end']:g}s: {prose(row['title'])}"
            )
    lines += ["", "## Semantic tree", ""]
    for row in rows:
        depth = LEVELS.index(row["level"])
        lines += [
            f"{'#' * (depth + 3)} {row['level'].title()} `{row['node_id']}` · "
        ...
        lines += [
            ...
            + ", ".join(evidence[:8])
            + (
                f" … ({len(evidence)} total; see nodes.jsonl)"
                if len(evidence) > 8
                else ""
            ),
            "",
...
class HierarchyBuilder:
    def __init__(
        self, video, duration, cfg, checkpoint, staging, client, check_cancelled
    ):
        ...
        for stamp in sorted(snapped):
            ...
            if frame_id not in self.frames:
                ...
                if image is None:
                    ...
                    extract_frame(
                        self.video, max(0, stamp - 1e-6), image, self.cfg["ingest"]
                    )
    ...
    def request(self, request_id, spec, prompt, frames, parser):
        ...
        if self.request_count > self.options["max_requests"]:
            raise HarnessError(
                "Hierarchy max_requests exhausted; no partial Wiki was published"
            )
        ...
        if saved is not None:
            if saved["input"] != identity:
                raise HarnessError(
                    f"Hierarchy checkpoint request changed: {request_id}"
                )
            ...
            for repair in range(self.cfg["ingest"]["caption_max_repairs"] + 1):
                ...
                attempt = {
                    ...
                    "model": self.cfg["ingest"]["vlm"]["model"],
                    "transport": {
                        key: self.cfg["ingest"]["vlm"].get(key)
                        for key in (
                            "provider",
                            "base_url",
                            "api_key_env",
                ...
                try:
                    ...
                    if (
                        not isinstance(exc, HarnessError)
                        or "raw_response" not in attempt
                        or repair == self.cfg["ingest"]["caption_max_repairs"]
                    ):
                        raise
                    correction = str(exc)
                    continue
                ...
                self.checkpoint.write(
                    name, {"input": identity, "raw_response": response}
                )
                break
        self.audit.append(
            {
    ...
    def split(self, start, end, depth, path, context, parent=None):
        ...
        stamps = mixed_times(
            *context, self.duration, self.scores, self.options, global_scan=depth == 0
        )
        ...
        segments = self.request(
            "split_" + path,
            spec,
            prompt,
            frames,
            lambda text: parse_split(
        ...
        for index, segment in enumerate(segments):
            ...
            if self.node_count > self.options["max_nodes"]:
                raise HarnessError(
                    "Hierarchy max_nodes exhausted; no partial Wiki was published"
                )
            child_path = f"{path}_{index + 1:03d}"
            node = {
                "node_id": f"{LEVELS[depth]}_{child_path}",
                "level": LEVELS[depth],
                **segment,
                "evidence_frame_ids": [
                    f["frame_id"]
            ...
            if not node["evidence_frame_ids"]:
                closest = sorted(
                    frames,
                    key=lambda f: min(
                        abs(f["timestamp"] - segment["start"]),
                        abs(f["timestamp"] - segment["end"]),
                    ),
                )[:2]
            ...
            if depth + 1 >= self.options["max_depth"]:
                ...
                bounds = context_bounds(
                    segments, index, start, end, self.options["overlap_ratio"]
                )
                node["children"] = self.split(
                    segment["start"],
                    segment["end"],
                    depth + 1,
                    child_path,
            ...
            result.append(node)
        return (
            self.merge(result, path)
            if self.options["merge_adjacent"] and len(result) > 1
            else result
        )
    ...
    def merge(self, siblings, path):
        ...
        frames = self.images(
            mixed_times(start, end, self.duration, self.scores, self.options)
        )
        inputs = [
            {k: v for k, v in node.items() if k != "children"}
            | {
                "children": [
                    {
                        k: child[k]
                        for k in ("node_id", "start", "end", "title", "summary")
                    }
        ...
        for index, group in enumerate(groups):
            ...
            node = {
                "node_id": f"{members[0]['level']}_merged_{path}_{index + 1:03d}",
                ...
                **{key: group[key] for key in SEMANTIC_KEYS},
                ...
                "evidence_frame_ids": list(
                    dict.fromkeys(f for m in members for f in m["evidence_frame_ids"])
                ),
                "children": [
                    child for member in members for child in member["children"]
                ],
            }
            if not node["children"]:
    ...
    def build(self):
        ...
        if changes is None:
            changes = {
                "scores": analyze_changes(
                    self.video, self.duration, self.options, self.check
                ),
        ...
        atomic_text(
            self.staging / "wiki.md", render_tree(rows, self.duration, len(frames))
        )
        attempts = [
            attempt for request in self.audit for attempt in request["attempts"]
        ]
        requests = [request for attempt in attempts for request in attempt["requests"]]
        return {
            "analysis_sec": changes["elapsed_sec"],
            "reused_analysis": reused_analysis,
            "extraction_sec_this_run": self.extraction_sec,
            "extraction_sec": sum(
                self.checkpoint.read(f["frame"] + ".json")["elapsed_sec"]
                for f in frames
            ),
            ...
            "reused_windows": self.reused_requests,
            "window_count": self.request_count,
            "node_count": len(rows),
...
def publish_hierarchy(
    ...
    try:
        ...
        logging.getLogger(__name__).warning(
            "Wiki published; could not remove checkpoint: %s", checkpoint.root
        )
    return metadata
